...
public class DictionaryService {
    ...
    @Transactional
    public void saveMovieMemberDic(Member member, Movie movie,int rating) {
        ...
        try {
            tmp = movie.getActor().split("\\|");
            for (String s : tmp) {
                ret_map.put(s, ret_map.get(s) + rating);
            }
        } catch (NullPointerException e) {
        }
        ...
        List<Movie_Genre> genreList = movie.getGenreList();
        for (Movie_Genre movie_genre : genreList) {
            ret_map.put(movie_genre.getGenre().getGenre(), ret_map.get(movie_genre.getGenre().getGenre()) + rating);
        }
        ...
        if (movie.getOutline().length() >= 1) {
            ...
            for (KoreanTokenJava s : strings) {
                if (s.getPos().toString().equals("Noun") ||
                        s.getPos().toString().equals("Alpha")) {
                    ret_map.put(s.getText(), ret_map.get(s.getText()) + rating);
                }
            }
        }
